...
public class CustomBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    private static final String TAG ="VehicleBottomSheetBehavior";
    ...
    // rjq+: 添加的expandGap和collapsedGap控制展开收起的条件
    private final float expandGap = 0.3f;
    private final float collapsedGap = 0.7f;
    int expandedOffset;
    ...
    public boolean onLayoutChild(
        ...
        View scrollChildView = findScrollingChild(child);


        nestedScrollingChildRef = new WeakReference<>(scrollChildView);
        return true;
    }
    ...
    public void onStopNestedScroll(
        ...
        int targetState;
        int currentTop = child.getTop();
        // 总可滑动范围是 collapsedOffset - getExpandedOffset()（即折叠位置与完全展开位置的差值）
        if (currentTop < ((collapsedOffset - getExpandedOffset()) * expandGap)) {
            top = this.getExpandedOffset();
            targetState = STATE_EXPANDED;
        } else if (currentTop > (collapsedOffset - getExpandedOffset()) * collapsedGap) {
